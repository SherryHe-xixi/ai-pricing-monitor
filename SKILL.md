...
**优先级 1 — Claude in Chrome MCP（推荐首选，DOM 级精确抓取）**
- 工具: `mcp__Claude_in_Chrome__tabs_context_mcp`（获取 tab）→ `mcp__Claude_in_Chrome__navigate`（打开页面）→ 等待 2-3 秒 → `mcp__Claude_in_Chrome__get_page_text`（提取文本）
- 先测试可用性：调用 `tabs_context_mcp(createIfEmpty=true)` 检查 Chrome 扩展是否已连接
- 如果已连接 → 逐个打开定价页并提取文本内容，可计算 content_hash 做精确变动检测
- **不受 egress proxy 限制**，能访问 WebFetch 被封锁的域名（已验证 mistral.ai、ai.google.dev 等均可访问）
- 注意：需要用户电脑开机且 Chrome 运行中
- 每个页面加载后等待 2-3 秒再提取文本，确保动态内容渲染完成
- 如果 Chrome 扩展未连接 → 降级到优先级 2

**优先级 2 — WebFetch（快速但受限）**
...
- 如果大面积失败（egress proxy 封禁）→ 降级到优先级 3
- **已知限制**：截至 2026-04-07，绝大多数域名被 egress proxy 封锁，仅 github.com 和 claude.com 可用

**优先级 3 — WebSearch（兜底方案，适用于电脑关机时的定时扫描）**
- 对每个品类执行 `"[品牌] pricing change [year]"` 搜索
- 只能捕捉有新闻报道的较大变动，小幅调价可能遗漏
- 无法计算 content_hash，跳过 Phase 1 直接做 Phase 2
- **延迟**：重大变动通常几小时内可搜到；小幅调价可能延迟数天或遗漏

**混合模式**：当 Chrome MCP 可用但部分页面加载失败时，对失败页面回退到 WebSearch 补充搜索。
...
#### 生成 Excel 快照
每次扫描完成后，使用 Python (openpyxl) 生成一份 Excel 文件，包含三个 sheet：
1. **产品定价总览**：所有产品的档位、价格、品类、区域，按 scan_priority 排序，支持筛选
2. **品类统计摘要**：各品类的产品数、自动扫描数、海外/国内分布
3. **扫描日志**：追加本次扫描的覆盖数、变动数、扫描方式等

文件命名规则：
- 按日期存档：`exports/AI定价监控_YYYY-MM-DD.xlsx`（不覆盖）
- 最新版覆盖：`AI定价监控_最新.xlsx`（每次覆盖）

保存位置：
- `~/AI产品PM landing/ai-pricing-monitor/exports/`（GitHub 仓库内）
- `~/AI产品PM landing/AI定价监控_最新.xlsx`（方便直接查看）
